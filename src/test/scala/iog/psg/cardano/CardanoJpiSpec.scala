...
class CardanoJpiSpec
  ...
  private val addressNotFoundError =
    "iog.psg.cardano.jpi.CardanoApiException: Message: Addresses not found, Code: 404"
  private val walletNotFoundError =
    "iog.psg.cardano.jpi.CardanoApiException: Message: Wallet not found, Code: 404"
  ...
  it should "fail on invalid request body" in {
    ...
    ) shouldBe "iog.psg.cardano.jpi.CardanoApiException: Message: Invalid binary string, Code: 400"
  }
  ...
  "POST /wallets/{walletId}/migrations" should "submit one or more transactions which transfers all funds from a Shelley wallet to a set of addresses" in {
    api
      .migrateShelleyWallet(wallet.id, walletPassphrase, unUsedAddresses.map(_.id).asJava)
      .toCompletableFuture
      .get()
  ...
  it should "return not found" in {
    tryGetErrorMessage(
      api.migrateShelleyWallet("invalid_address_id", walletPassphrase, unUsedAddresses.map(_.id).asJava)
    ) shouldBe walletNotFoundError
  }
  ...
  "GET /wallets/{walletId}/migrations" should "calculate the exact cost of sending all funds from particular Shelley wallet to a set of addresses" in {
    api
      .getShelleyWalletMigrationInfo(wallet.id)
      .toCompletableFuture
